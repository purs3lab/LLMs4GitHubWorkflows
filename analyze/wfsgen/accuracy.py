...
def calculate_passed(csv_filename):
    passed_count = 0
    total_count = 0
    with open(csv_filename, 'r') as file:
        ...
        for row in csv_reader:
            total_count += 1
            if "passed" in row[1:]:  # Check if any result is "passed"
                passed_count += 1
...
def main(dir):
    ...
    for csv_filename in csv_files:
        ...
        if model_name not in results:
            results[model_name] = []
        passed_count, total_count = calculate_passed(csv_filename)
        accuracy = round(passed_count / total_count * 100, 2) 
        accuracy = str(accuracy).ljust(5, '0')
        results[model_name].append((temperature, prompt, accuracy))
